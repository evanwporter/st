...
xinit(int cols, int rows)
{
	...
	/* Load the xterm cursor from the active Xcursor theme. */
	cursor = XcursorLibraryLoadCursor(xw.dpy, "xterm");
...
xmakeglyphfontspecs(XftGlyphFontSpec *specs, const Glyph *glyphs, int len, int x, int y)
{
	...
	hb_buffer_clear_contents(shaped.buffer);
	return numspecs;
}
...
{
	int dmode, i, x, ox, numspecs;
	Glyph base, new;
	XftGlyphFontSpec *specs = xw.specbuf;

	/* Draw backgrounds before foregrounds so wide glyphs cannot be clipped by
	 * the background of an adjacent attribute run. */
	for (dmode = DRAW_BG; dmode <= DRAW_FG; dmode <<= 1) {
		i = ox = 0;
		for (x = x1; x < x2; x++) {
			new = line[x];
